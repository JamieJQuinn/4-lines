...
function setup() {
  ...
  settings.addButton("Clear Screen", function() {
...
function draw() {
  if(clearScreenOnce) {
    var temp = newColor(bg_colour, 255);
    background(temp);
  ...
  if (mouseIsPressed) {
    if (mouseButton == LEFT) {
      ...
      pendulum.setEnd(xPos, yPos);

      var pos1 = pendulum.pos1();
      var pos2 = pendulum.pos2();
      stroke(pendulum.colour);
      strokeWeight(1);
      if (showPendulum1Enabled) {
        line(SCALE*pendulum.origin[0], SCALE*pendulum.origin[1], SCALE*pos1[0], SCALE*pos1[1]);
      }
...
DoublePendulum.prototype.setEnd = function(xPos, yPos) {
  var alpha = atan2(xPos, yPos);
  var r = Math.sqrt(xPos*xPos + yPos*yPos);
  if (r >= this.l1 + this.l2) {
    if(r >= this.l1+this.l2 + 10) {
      return;
    }
    this.theta1 = this.theta2 = alpha;
  } else if (this.l1 > this.l2 && r <= this.l1 - this.l2) {
    this.theta1 = alpha;
    this.theta2 = alpha+PI;
  } else if (this.l2 > this.l1 && r <= this.l2 - this.l1) {
    this.theta1 = alpha+PI;
    this.theta2 = alpha;
  } else {
    var x2 = (Math.pow(this.l2, 2) - Math.pow(this.l1, 2) + Math.pow(r, 2))/(2*r);
    var x1 = r - x2;
    this.theta1 = alpha + Math.acos(x1/this.l1);
    this.theta2 = alpha - Math.acos(x2/this.l2);
  }
  this.reset();
}
